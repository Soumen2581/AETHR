...
| TD-2 | Full editor `repaint()` at 36 Hz | UI CPU | Medium | Dirty-region / component timers | P2 | Mitigated (Sprint 2) |
| TD-3 | Delay time jumps without crossfade | Automation clicks | High | Crossfade reads in `FxRack` | P1 | Closed (Sprint 1) |
| TD-4 | SVF filter coeffs recomputed every sample | CPU | Medium | Block-rate coeff update | P2 | Closed (Sprint 3) |
...
| TD-6 | Chorus + phaser share `lfoPhase` | Coupled motion | Low | Separate phases | P2 | Closed (Sprint 3) |
...
| TD-14 | Modulation matrix read-only | Looks interactive | Low | Relabel or wire | P2 | Closed (ROUTING OVERVIEW) |
